...
class GemmaTransformer(nn.Module):
    """
    Gemma version of the LinearTransformer.
    - Operates on vector tokens (context_x, target_x)
    - Preserves original API (_predict_single, forward, compute_in_context_preds)
    """
    def __init__(self, d: int, model_name="google/gemma-3-27b-it"):
        super().__init__()
        self.d = d

        # Load pretrained Gemma backbone
        self.gemma = GemmaModel.from_pretrained(model_name)

        H = self.gemma.config.hidden_size
    ...
    def _gemma_encode(self, tokens):
        x = self.input_proj(tokens)  # (B, T, H)
        out = self.gemma(inputs_embeds=x).last_hidden_state
        return out
...
class Trainer:
    ...
    def __init__(self, config: ExperimentConfig):
        ...
        self.model = GemmaTransformer(config.d).to(config.device)
