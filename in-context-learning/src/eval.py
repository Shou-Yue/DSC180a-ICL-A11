...
def model_device(model):
    """Return the device for torch models, else CPU for non-torch baselines."""
    if isinstance(model, nn.Module):
        ...
        return get_device()
    # Non-torch baselines (e.g., LeastSquaresModel) run on CPU
    return torch.device("cpu")

def get_device():
    """
    Prefer MPS on Apple Silicon, then CUDA, else CPU.
    Handles older PyTorch builds that may not expose torch.backends.mps.
    """
...
def to_device(obj, device):
    """
    Move tensors (or collections of tensors) to device. No-op for non-tensors.
    """
...
def get_model_from_run(run_path, step = -1, only_conf = False):
    ...
    with open(config_path) as fp:  # avoid Quinfig to skip inherits
    ...
    # Always load checkpoints onto CPU (safe even if they were saved on CUDA)
    map_cpu = torch.device("cpu")

    if step == -1:
        state_path = os.path.join(run_path, "state.pt")
        # Prefer safe load to avoid pickle execution when possible
        try:
            state = torch.load(state_path, map_location = map_cpu, weights_only = True)
        except TypeError:
            warnings.warn(
                "Using torch.load with weights_only=False fallback; ensure the file is trusted."
            )
            state = torch.load(state_path, map_location = map_cpu)
        state_dict = state["model_state_dict"]
    else:
        model_path = os.path.join(run_path, f"model_{step}.pt")
        try:
            state_dict = torch.load(model_path, map_location = map_cpu, weights_only = True)
        except TypeError:
            warnings.warn(
                "Using torch.load with weights_only=False fallback; ensure the file is trusted."
            )
            state_dict = torch.load(model_path, map_location = map_cpu)

    # If saved with DataParallel / DDP, strip the "module." prefix
    ...
    if xs_p is None:
        ...
        for i in range(n_points):
            ...
            if is_torch_model:
                ...
                pred = model(xs_comb, ys, inds=[i])
                metrics[:, i] = task.get_metric()(pred, ys)[:, i]
    return metrics
...
def get_run_metrics(
    ...
    if skip_model_load:
        ...
        # model already moved to device and set to eval in get_model_from_run
        all_models = [model]
        if not skip_baselines:
            # Baselines may be CPU-only; the eval path handles CPU tensors
...
def conf_to_model_name(conf):
    if conf.model.family in ["gpt2", "llama"]:
        return {
            (3, 2): "Transformer-xs",
