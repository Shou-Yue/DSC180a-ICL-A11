...
class GaussianMixtureDataset(Dataset):
    ...
    def __init__(
        ...
        label_flip_p: float = 0.0,
        base_seed: int = 42,  # NEW: local base seed
    ):
        self.d = d
        self.N = N
        self.B = B
        ...
        self.label_flip_p = float(label_flip_p)
        assert 0 <= self.label_flip_p < 0.5, f"label flip probab must be in [0,1/2), got {label_flip_p}"

        # --- NEW: deterministic local RNGs; no global torch.manual_seed here ---
        # If you want val to be different but still deterministic, offset its seed.
        offset = 10_000 if is_validation else 0
        self._g_base = torch.Generator(device="cpu")
        self._g_base.manual_seed(int(base_seed) + offset)

        self._g_flip = torch.Generator(device="cpu")
        self._g_flip.manual_seed(int(base_seed) + 1 + offset)

        # Generate all data once and store in tensors
    ...
    def _generate_data(self) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        Returns:
            ...
            target_y: (B,)
        """
        # ----- base data (ONLY self._g_base) -----
        mus = torch.randn(self.B, self.d, generator=self._g_base)                     # (B, d)
        mus = mus / torch.norm(mus, dim=1, keepdim=True) * self.R

        # Clean labels (0/1) for N+1 samples per task
        y_all = (torch.rand(self.B, self.N + 1, generator=self._g_base) > 0.5).float()  # (B, N+1)
        y_signal = 2 * y_all - 1  # {-1, +1}

        # Gaussian noise for features
        z = torch.randn(self.B, self.N + 1, self.d, generator=self._g_base)           # (B, N+1, d)

        # Features depend only on clean labels + mus + noise
        x = y_signal[..., None] * mus[:, None, :] + z                                 # (B, N+1, d)

        # ----- label flips (ONLY self._g_flip) -----
        if self.label_flip_p > 0.0:
            flip_mask = torch.rand(self.B, self.N + 1, generator=self._g_flip) < self.label_flip_p
